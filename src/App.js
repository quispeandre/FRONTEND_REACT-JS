...
function App() {
  ...
  const fetchProductos = useCallback(async () => {
    try {
      const respuesta = await fetch(`${process.env.REACT_APP_API_URL}/api/autores`);
  ...
  const EditarProducto=async(e)=>{
    ...
    try{
     const respuesta=await fetch(`${process.env.REACT_APP_API_URL}/api/autores/${productoId}`,{
        method:'PUT',
        headers:{'Content-Type':'application/json'},
        body: JSON.stringify({
          ...formularioEditar
  ...
  const EliminarProducto=async(id)=>{
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try{
          await fetch(`${process.env.REACT_APP_API_URL}/api/autores/${id}`,{
            method:`DELETE`
          });
          Swal.fire({
   ...
   const VerLibros=async(id)=>{
      try{
         const respuesta=await fetch(`${process.env.REACT_APP_API_URL}/api/autores/${id}`);
         const data = await respuesta.json();
         SetLibros(data);
         AbrirVentana();
      }catch(error){
